...
function wireEvents() {
  ...
  els.loginForm.addEventListener("submit", async event => {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      ...
      options: { emailRedirectTo: CONFIG.loginRedirectUrl }
    });
...
function filteredNotices() {
  ...
  const result = notices.filter(n => {
    ...
    if (status !== "all" && displayStatus(n) !== status) return false;
...
function renderStats() {
  ...
  const newCount = notices.filter(n => n.isNew || sameDate(n.firstCollectedAt, today)).length;
...
function updateCounts() {
  const stageCounts = { all: notices.length, favorite: 0, "검토 필요": 0, "신청 예정": 0, "서류 준비 중": 0, "신청 완료": 0 };
  notices.forEach(n => {
